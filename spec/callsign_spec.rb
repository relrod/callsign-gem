...
describe Callsign do
  ...
  it "backward compatibility, should return valid info for a USA callsign" do
    result = Callsign.new 'w1aw'
    result.name.start_with?('ARRL').should == true
  end

  it "backward compatibility, should raise an exception on invalid callsign" do
    lambda { Callsign.new('w333333w') }.should raise_error(InvalidCallsignException)
  end

  it "backward compatibility, should raise an invalid http response exception if we get a 404 back" do
